...
(() => {
    ...
    function getChatContext() {
      const chatMessages = document.querySelectorAll('[data-element-id^="chat-message-"]');
      return Array.from(chatMessages).slice(-3).map(msg => msg.textContent).join("\n");
    }
      ...
      generateButton.onclick = async () => {
        if (isExtensionEnabled()) {
          ...
          try {
            ...
            displaySuggestions(suggestions[0].generated_text.split('\n').filter(s => s.trim()));
          } catch (error) {
